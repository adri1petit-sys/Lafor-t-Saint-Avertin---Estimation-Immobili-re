...
const AGENCY_PLACE_ID = "ChIJz-lxxvnv_EcRq7p86t9s-z0"; // Google Places ID for "Laforêt Saint-Avertin"
          ...
          (place, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK && place && place.reviews) {
              const fetchedReviews = place.reviews
                .filter(review => review.text && typeof review.rating === 'number') // Ensure reviews have content
