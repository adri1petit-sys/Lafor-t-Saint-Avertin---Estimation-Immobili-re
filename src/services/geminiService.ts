...
import { EstimationFormData, EstimationResult, ChatMessage } from "../types/types";
...
export const getRealEstateEstimation = async (formData: EstimationFormData): Promise<EstimationResult> => {
  const prompt = `
    ...
    try {
        // FIX: Added a check for an empty response to prevent crash on .trim()
...
// FIX: Re-implemented chatWithBot to be stateless and correctly handle conversation history.
// The previous implementation created a new chat session for each message, losing context.
// Using generateContent with the full history is more suitable for this app's architecture.
export const chatWithBot = async (history: ChatMessage[]): Promise<string> => {
    ...
    return response.text ?? "Désolé, une erreur est survenue. Je n'ai pas pu générer de réponse.";
};
